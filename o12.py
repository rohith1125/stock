...
# Step 2: Implement the PRISM Algorithm
    ...
    for feature in features:
        ...
        if data[feature].dtype == 'object':
            ...
            for value in unique_values:
                ...
                if len(subset) == 0:
                    continue
                accuracy = sum(subset['NextDayUp'] == target_class) / len(subset)
            ...
            # Consider thresholds at each unique value
...
def prism_algorithm(data, target_class):
    ...
    while len(data_remaining[data_remaining['NextDayUp'] == target_class]) > 0:
        ...
        while True:
            best_condition, max_accuracy, best_subset = find_best_condition(data_subset, target_class)
            ...
            if max_accuracy == 1.0:
                break  # Perfect rule
        # Store the rule
        rule = {'conditions': rule_conditions, 'accuracy': max_accuracy, 'coverage': len(data_subset)}
        rules.append(rule)
