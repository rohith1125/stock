...
# Combine rules using logical AND/OR operations
def combine_rules(rules):
    combined_rules = []
    for rule1, rule2 in combinations(rules, 2):
        combined_rule = rule1[0] + rule2[0]
        combined_coverage = rule1[1] + rule2[1]
        combined_accuracy = (rule1[2] + rule2[2]) / 2
        combined_rules.append((combined_rule, combined_coverage, combined_accuracy))
...
if __name__ == "__main__":
    ...
    # Combine rules using logical AND/OR for better generalization
    combined_rules = combine_rules(refined_rules)
