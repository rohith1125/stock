...
def prism_algorithm_direct(training_data, target_column, target_value, features):
    """
    Implements the Prism algorithm using original feature values without modifications.
...
# Generate rules for the target value 1 (YES) using original feature values
try:
    direct_rules = prism_algorithm_direct(training_data_processed, 'Target', 1, features)
except Exception as e:
    print(f"Error in rule generation: {e}")

# Print generated rules with coverage and accuracy
print("\nGenerated Rules:")
for rule_idx, (rule, coverage, accuracy) in enumerate(direct_rules, start=1):
