...
print(training_data['NextDayUp'].value_counts(normalize=True))

print("\nValidation Data Class Distribution:")
print(validation_data['NextDayUp'].value_counts(normalize=True))

# Step 5: Implementing the Optimized PRISM Algorithm
...
def find_best_condition(data, target_class):
    """
    Find the best condition that maximizes information gain.
    """
    ...
    for feature in features:
        if data[feature].dtype == 'object' or data[feature].dtype.name == 'category':
            ...
            for threshold in thresholds:
                ...
                subset = data[condition]
                if len(subset) < 5 or len(subset) == len(data):
                    continue
                info_gain = calculate_information_gain(data, subset, 'NextDayUp')
                if info_gain > best_info_gain:
                    best_info_gain = info_gain
...
def prism_algorithm(data, target_class, min_coverage=5, max_conditions=3):
    """
    ...
    while len(data_remaining[data_remaining['NextDayUp'] == target_class]) >= min_coverage:
        ...
        for _ in range(max_conditions):
            best_condition, best_info_gain, best_subset = find_best_condition(data_subset, target_class)
...
# Step 6: Generate Rules Using PRISM Algorithm

# Define the target class
target_class = 1  # Predicting NextDayUp = 1
...
# Step 7: Apply Rules to Validation Data and Make Predictions
